...
class Ekf(object):
    ...
    def measurement_update(self, z_raw, Q_raw):
        ...
        ########## Code starts here ##########
        # TODO: Update self.x, self.Sigma.
        St = H.dot(self.Sigma).dot(H.T) + Q
        Kt = self.Sigma.dot(H.T).dot(np.linalg.inv(St))
        self.x += Kt.dot(z).reshape(-1)
        self.Sigma -= Kt.dot(St).dot(Kt.T)
        ########## Code ends here ##########
...
class EkfLocalization(Ekf):
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        z = np.array(v_list).reshape(-1)  # v_list: [np.array[2,]]
        H = np.array(H_list)
        H = H.reshape(-1, H.shape[2])  # H_list: [np.array[2,3]]
        ########## Code ends here ##########
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for i in range(I):
            ...
            for j in range(J):
                ...
                if dij < dmin:
                    dmin, v, Q, H = dij, vij, Q_raw[i], Hs[j]
            if dmin < self.g ** 2:
                v_list.append(v)
                Q_list.append(Q)
                H_list.append(H)
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(self.map_lines.shape[1]):
            ...
            h, Hx = tb.transform_line_to_scanner_frame(self.map_lines[:, j],
                                                       self.x, self.tf_base_to_camera, compute_jacobian=True)
            ########## Code ends here ##########
...
class EkfSlam(Ekf):
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            map_line = np.array([alpha, r])
            h, Hx[:, 0:3] = tb.transform_line_to_scanner_frame(map_line,
                                                               self.x[0:3], self.tf_base_to_camera,
                                                               compute_jacobian=True)
            if j >= 2:
                x_w, y_w, theta_w = self.x[0:3]
                x_cam, y_cam, _ = self.tf_base_to_camera
                x_cam_w = x_cam * np.cos(theta_w) - y_cam * np.sin(theta_w) + x_w
                y_cam_w = x_cam * np.sin(theta_w) + y_cam * np.cos(theta_w) + y_w
                Hx[:, idx_j:idx_j + 2] = np.array([[1, 0], [x_cam_w * np.sin(alpha) - y_cam_w * np.cos(alpha), 1]])
            ########## Code ends here ##########
